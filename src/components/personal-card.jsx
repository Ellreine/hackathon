...
const PersonCard = ({
	...
	return (
		<div className="w-[336px] min-h-[420px] relative card z-0">
			...
			<div className="back w-full h-full overflow-hidden absolute transform transition-transform duration-500 ease-linear shadow-2xl hover:shadow-none rounded-2xl gradient">
				<div className="w-full h-full text-center text-gray-200 relative text-lg flex justify-around flex-col items-center">
					<div className="w-full h-[150px] absolute top-[0%] gradientLight">
						<img
						...
						/>
						<div className="h-[90px] w-[90px] rounded-full mx-auto mt-3 overflow-hidden">
							<img src={imageUrl} alt="" />
						</div>

						<h2 className="text-[30px] text-[#d6e8ee] text-center font-[600] mt-3">
							{name}
						</h2>
					</div>
					<div
						className={` w-full h-[150px] absolute top-[40%] flex justify-center items-center flex-col`}
					>
						<h3 className="text-[22px] mt-1 font-bold text-yellow-400">{profession}</h3>
						<span className="text-[20px] ">Age: {age}</span>
						<span className="text-[20px]">City: {city}</span>
						<h3 className="text-[20px] w-[85%] h-[150px] mt-7">{about}</h3>
					</div>
					<Link
						to={id}
